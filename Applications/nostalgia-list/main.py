...
def get_track_ids(tracks):
    ...
    for track in track_list_response:
        try:
            track_ids.append(track["tracks"]["items"][0]['id'])
...
def create_playlist(playlist, playlist_ids):
    print(playlist)
    print(playlist_ids)
    ...
    # playlist_name = f"Nostalgia {year[:4]}"
    #
    ...
    #                         name=playlist_name,
    #                         public=True,
    #                         description=f"Top tracks for {year}")
    #
    # sp.playlist_add_items(sp.current_user()["id"])
...
# Create a playlist name for the year entered.
playlist_name = f"Nostalgia {year[:4]}"
...
# Create the playlist
create_playlist(playlist_name, ids)
